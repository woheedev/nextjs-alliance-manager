...
async function fetchAllStatics() {
  ...
  console.log(
    `[API] Found ${total} total static assignments to fetch for preset 2`
  );
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log(
      `[API] Updating static group (preset 2) for ${discordId} to ${
    ...
    if (existingDocs.documents.length > 0) {
      ...
      if (group === null) {
        // Remove from group
        console.log(
          `[API] Removing member ${discordId} from static group (preset 2)`
        );
        ...
        // Update group
        console.log(
          `[API] Updating member ${discordId} to group ${group} (preset 2)`
        );
      ...
      console.log(
        `[API] Creating new static group entry for member ${discordId} in group ${group} (preset 2)`
      );
    ...
    console.error("[API ERROR] Error updating static group (preset 2):", error);
    return NextResponse.json(
      {
